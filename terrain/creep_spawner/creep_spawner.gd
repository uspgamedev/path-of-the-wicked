...
func get_next_point(pos, creep):
	if creep.spawn_path.size() == 0:
		creep.spawn_path = map.update_path(creep.spawn_graph, creep.spawn_path, creep.spawner.position, map.base)
	...
	if creep.path == null:
		creep.path = map.update_path(creep.spawn_graph, creep.path, pos, map.base)
